...
{
	/// <summary>
	///  A dynamic engine used to access and create objects dynamically.
	/// <remarks>
	///  It uses <see cref="System.Reflection.Emit"/> to generate the access methods, holds all dynamic methods in its cache for next use, and it's thread safe.
	/// </remarks>
	/// <example>
	/// 1.Access Property
	...
	///  yohan zhou 
	/// </author>
	public class DynamicAccessEngine
	{
		#region Field
		...
		/// <summary>
		/// Cleans all cache dynamic methods.
		/// </summary>
		...
		/// <returns></returns>
		public static Func<T, Result> GetPropertyDelegate<T, Result>(string propName)
		{
			Type type = typeof(T);
			var method = PropertyGetterMethod(type, propName);
			return (Func<T, Result>)method.CreateDelegate(typeof(Func<T, Result>));
		}
		...
		/// <param name="propName">Name of the prop.</param>
		/// <returns></returns>
		public static Action<T> SetPropertyDelegate<T>(string propName)
		{
			Type type = typeof(T);
			var method = PropertySetterMethod(type, propName);
		...
		private static DynamicMethod CreateInstanceMethod(Type type, Type[] ptypes)
		{
			...
			if (!instanceCreators.ContainsKey(key))
			{
				lock (isSync)
				{
					DynamicMethod dm = new DynamicMethod(key, typeof(object), new Type[] { typeof(object[]) }, typeof(DynamicAccessEngine).Module, true);
		...
		private static DynamicMethod FieldGetterMethod(object instance, FieldInfo fieldInfo)
		{
			...
			if (!fieldGetters.ContainsKey(key))
			{
				lock (fgSync)
				{
					var getter = new DynamicMethod("__get_field_" + key, fieldInfo.FieldType, new[] { t }, typeof(DynamicAccessEngine), true);
		...
		private static DynamicMethod FieldSetterMethod(object instance, string fieldName)
		{
			...
			if (!fieldSetters.ContainsKey(key))
			{
				lock (fsSync)
				{
					...
					var setter = new DynamicMethod("__set_field_" + key, null, new[] { t, fieldInfo.FieldType }, typeof(DynamicAccessEngine), true);
		...
		{
			var t = instance.GetType();
			return PropertyGetterMethod(t, propertyName);
		}
		...
		/// <param name="propertyName">Name of the property.</param>
		/// <returns></returns>
		private static DynamicMethod PropertyGetterMethod(Type type, string propertyName)
		{
			...
			if (!propertyGetters.ContainsKey(key))
			{
				lock (pgSync)
				{
					...
					if (propertyInfo == null)
						throw new ArgumentException("There is no publicly accessible " + propertyName + " property found in " + type.FullName);

					if (!propertyInfo.CanRead)
						throw new ArgumentException("The property " + propertyName + " has no publicly accessible getter.");

					var getter = new DynamicMethod("__get_property_" + key, propertyInfo.PropertyType, new[] { type }, typeof(DynamicAccessEngine), true);
		...
		/// <summary>
		/// Properties the setter method.
		/// </summary>
		/// <param name="t">The t.</param>
		/// <param name="propertyName">Name of the property.</param>
		/// <returns></returns>
		private static DynamicMethod PropertySetterMethod(Type t, string propertyName)
		{
			...
			if (!propertySetter.ContainsKey(key))
			{
				lock (psSync)
				{
					...
					var setter = new DynamicMethod("__set_property_" + key, null, new[] { t, propertyInfo.PropertyType }, typeof(DynamicAccessEngine), true);
		...
		{
			var t = instance.GetType();
			return PropertySetterMethod(t, propertyName);
		}
