...
func main() {
	if len(os.Args) != 3 {
		fmt.Println("Usage:", os.Args[0], "<local_ip> <remote_ip")
		os.Exit(1)
	}

	localAddr, err := net.ResolveIPAddr("ip4", os.Args[1])
	if err != nil {
		fmt.Println("LocalAddr Resolution error", err.Error())
		os.Exit(1)
	}

	remoteAddr, err := net.ResolveIPAddr("ip4", os.Args[2])
	if err != nil {
		fmt.Println("RemoteAddr Resolution error", err.Error())
		os.Exit(1)
	}
