...
// -----------------------------------------------------
// Max9 has no "VertexNormal.h", make it here
class VNormal
{
private:
    Point3 norm_;
...
namespace
{
	...
	template <> inline bool KeysEqual<ILinPoint3Key>(const ILinPoint3Key& key1, const ILinPoint3Key& key2)
	{
	...
	template <> inline bool KeysEqual<IBezPoint3Key>(const IBezPoint3Key& key1, const IBezPoint3Key& key2)
	{
		return ((fabs(key1.val.x - key2.val.x) < kExportEpsilon) && (fabs(key1.val.y - key2.val.y) < kExportEpsilon) && (fabs(key1.val.z - key2.val.z) < kExportEpsilon));
	}

	template <> inline bool KeysEqual<ITCBPoint3Key>(const ITCBPoint3Key& key1, const ITCBPoint3Key& key2)
	{
		return ((fabs(key1.val.x - key2.val.x) < kExportEpsilon) && (fabs(key1.val.y - key2.val.y) < kExportEpsilon) && (fabs(key1.val.z - key2.val.z) < kExportEpsilon));
	}

	template <> inline bool KeysEqual<ILinRotKey>(const ILinRotKey& key1, const ILinRotKey& key2)
	{
	...
	template <> inline bool KeysEqual<IBezQuatKey>(const IBezQuatKey& key1, const IBezQuatKey& key2)
	{
		return ((fabs(key1.val.x - key2.val.x) < kExportEpsilon) && (fabs(key1.val.y - key2.val.y) < kExportEpsilon) && (fabs(key1.val.z - key2.val.z) < kExportEpsilon) && (fabs(key1.val.w - key2.val.w) < kExportEpsilon));
	}

	template <> inline bool KeysEqual<ITCBRotKey>(const ITCBRotKey& key1, const ITCBRotKey& key2)
	{
	...
	template <> inline bool KeysEqual<ILinScaleKey>(const ILinScaleKey& key1, const ILinScaleKey& key2)
	{
	...
	template <> inline bool KeysEqual<IBezScaleKey>(const IBezScaleKey& key1, const IBezScaleKey& key2)
	{
		return ((fabs(key1.val.s.x - key2.val.s.x) < kExportEpsilon) && (fabs(key1.val.s.y - key2.val.s.y) < kExportEpsilon) && (fabs(key1.val.s.z - key2.val.s.z) < kExportEpsilon) && (fabs(key1.val.q.x - key2.val.q.x) < kExportEpsilon) && (fabs(key1.val.q.y - key2.val.q.y) < kExportEpsilon) && (fabs(key1.val.q.z - key2.val.q.z) < kExportEpsilon) && (fabs(key1.val.q.w - key2.val.q.w) < kExportEpsilon));
	}

	template <> inline bool KeysEqual<ITCBScaleKey>(const ITCBScaleKey& key1, const ITCBScaleKey& key2)
	{
		return ((fabs(key1.val.s.x - key2.val.s.x) < kExportEpsilon) && (fabs(key1.val.s.y - key2.val.s.y) < kExportEpsilon) && (fabs(key1.val.s.z - key2.val.s.z) < kExportEpsilon) && (fabs(key1.val.q.x - key2.val.q.x) < kExportEpsilon) && (fabs(key1.val.q.y - key2.val.q.y) < kExportEpsilon) && (fabs(key1.val.q.z - key2.val.q.z) < kExportEpsilon) && (fabs(key1.val.q.w - key2.val.q.w) < kExportEpsilon));
	}
	...
	template <> inline bool KeyTangentsNonzero<IBezPoint3Key>(const IBezPoint3Key& key)
	{
	...
	template <> inline bool KeyTangentsNonzero<IBezScaleKey>(const IBezScaleKey& key)
	{
		return ((fabs(key.intan.x) > kExportEpsilon) || (fabs(key.intan.y) > kExportEpsilon) || (fabs(key.intan.z) > kExportEpsilon) || (fabs(key.outtan.x) > kExportEpsilon) || (fabs(key.outtan.y) > kExportEpsilon) || (fabs(key.outtan.z) > kExportEpsilon));
	}
	...
	template <> inline const Point3& GetKeyPoint3<ILinScaleKey>(const ILinScaleKey& key)
	{
		return (key.val.s);
	}

	template <> inline const Point3& GetKeyPoint3<IBezScaleKey>(const IBezScaleKey& key)
	{
		return (key.val.s);
	}

	template <> inline const Point3& GetKeyPoint3<ITCBScaleKey>(const ITCBScaleKey& key)
	{
		return (key.val.s);
	}
	...
	template <> inline const Quat& GetKeyQuat<ITCBRotKey>(const ITCBRotKey& key)
	{
		return (QFromAngAxis(key.val.angle, key.val.axis));
	}

	template <> inline const Quat& GetKeyQuat<ILinScaleKey>(const ILinScaleKey& key)
	{
		return (key.val.q);
	}

	template <> inline const Quat& GetKeyQuat<IBezScaleKey>(const IBezScaleKey& key)
	{
		return (key.val.q);
	}

	template <> inline const Quat& GetKeyQuat<ITCBScaleKey>(const ITCBScaleKey& key)
	{
		return (key.val.q);
	}
}
...
{
	return ("Open Game Engine Exchange File");
}
...
{
	exportFile = CreateFileA(name, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
	...
	{
		ProcessNode(root->GetChildNode(i));
	}
...
ExportVertex *OpenGexExport::DeindexMesh(Mesh *mesh, Mesh *baseMesh, int *exportTriangleCount, int *exportColorCount, int *exportTexcoordCount)
{
	...
	*exportTriangleCount = triangleCount;
	...
	VNormal *vnormal = new VNormal[vertexCount];
...
void OpenGexExport::ProcessNode(INode *node)
{
...
template <class keyType> void OpenGexExport::ExportFloatKeyValues(IKeyControl *keyInterface)
{
	IndentWrite("Key {float {");
...
void OpenGexExport::ExportFloatKeyValueControlPoints(IKeyControl *keyInterface)
{
	IBezFloatKey	key[2];
	...
	for (int i = 0;;)
	{
		...
		if (GetInTanType(key[1].flags) == BEZKEY_USER)
		{
			...
			WriteFloat(key[1].val + key[1].intan * dt);
		}
		else
		{
			WriteFloat(key[0].val * 0.333333F + key[1].val * 0.666667F);
		}
	...
	for (int i = 0;;)
	{
		...
		if (GetOutTanType(key[0].flags) == BEZKEY_USER)
		{
			...
			WriteFloat(key[0].val + key[0].outtan * dt);
		}
		else
		{
			WriteFloat(key[0].val * 0.666667F + key[1].val * 0.333333F);
		}
...
template <class keyType> void OpenGexExport::ExportPoint3KeyValueControlPoints(IKeyControl *keyInterface)
{
	...
	for (int i = 0;;)
	{
		...
		{
			WritePoint3(GetKeyPoint3(key[0]) * 0.333333F + GetKeyPoint3(key[1]) * 0.666667F);
		}
	...
	for (int i = 0;;)
	{
		...
		{
			WritePoint3(GetKeyPoint3(key[0]) * 0.666667F + GetKeyPoint3(key[1]) * 0.333333F);
		}
...
template <class keyType> void OpenGexExport::ExportFloatKeyData(IKeyControl *keyInterface, const char *kind, const float keyType::*data)
{
...
void OpenGexExport::ExportAnimationTrack(Control *control, const char *target, bool newline)
{
	...
	if (classID == LININTERP_FLOAT_CLASS_ID)
	{
		...
		ExportFloatKeyValues<ILinFloatKey>(keyInterface);
	...
	else if (classID == HYBRIDINTERP_FLOAT_CLASS_ID)
	{
		...
		ExportFloatKeyValues<IBezFloatKey>(keyInterface);
		ExportFloatKeyValueControlPoints(keyInterface);
	...
	else if (classID == TCBINTERP_FLOAT_CLASS_ID)
	{
		...
		ExportFloatKeyValues<ITCBFloatKey>(keyInterface);

		ExportFloatKeyData<ITCBFloatKey>(keyInterface, "tension", &ITCBFloatKey::tens);
		ExportFloatKeyData<ITCBFloatKey>(keyInterface, "continuity", &ITCBFloatKey::cont);
		ExportFloatKeyData<ITCBFloatKey>(keyInterface, "bias", &ITCBFloatKey::bias);
...
void OpenGexExport::ExportSampledAnimationTrack(INode *node)
{
...
void OpenGexExport::ExportTransform(INode *node)
{
	...
	if (control)
	{
		...
		if (positionControl)
		{
			if (positionControl->SuperClassID() == CTRL_POSITION_CLASS_ID)
			{
				...
				if ((posControl[0]->SuperClassID() == CTRL_FLOAT_CLASS_ID) && (posControl[1]->SuperClassID() == CTRL_FLOAT_CLASS_ID) && (posControl[2]->SuperClassID() == CTRL_FLOAT_CLASS_ID))
				{
		...
		if (rotationControl)
		{
			if (rotationControl->SuperClassID() == CTRL_ROTATION_CLASS_ID)
			{
				...
				if ((rotControl[0]->SuperClassID() == CTRL_FLOAT_CLASS_ID) && (rotControl[1]->SuperClassID() == CTRL_FLOAT_CLASS_ID) && (rotControl[2]->SuperClassID() == CTRL_FLOAT_CLASS_ID))
				{
	...
	if ((!positionAnimated) && (!rotationAnimated) && (!scaleAnimated))
	{
		...
		// Export a sampled animation track if necessary.

		if ((sampledAnimation) && (control->SuperClassID() == CTRL_MATRIX3_CLASS_ID))
		{
			ExportSampledAnimationTrack(node);
		}
	}
	else
	{
	...
	if ((texmap) && (texmap->ClassID().PartA() == BMTEX_CLASS_ID))
	{
		...
		if (name)
		{
			...
			if (gen)
			{
				...
				{
					Matrix3 matrix(Point3(uscale, 0.0F, 0.0F), Point3(0.0F, vscale, 0.0F), Point3(0.0F, 0.0F, 0.0F), Point3(uoffset, voffset, 0.0F));
...
void OpenGexExport::ExportMaterials(void)
{
	...
	if (count != 0)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			ExportTexture(material, ID_SH, "specular");
